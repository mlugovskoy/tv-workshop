...
const submit = async () => {
    ...
    try {
        const response = await fetch(`/api/devices/${route.params.id}`, {
            method: 'PUT',
            headers: {
        ...
        showSuccess('Устройство успешно изменено')
        router.push('/devices');
    } catch (error) {
...
const loadDevice = async () => {
    try {
        ...
        form.serial_number = response.data.serial_number;
...
<template>
    <div class="flex items-center justify-between">
        <PageTitle
            ...
            subtitle="Редактирование существующего в базе устройства"
        />
    </div>
